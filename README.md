...
M4의 고수준 계약인 결정 23은 승인됐지만 구현은 아직 승인되지 않았다. M4는 M3의 선택을 주관적 정보로 구체화한 뒤 객관적 현실에서 원자적으로 해소하며, 최초 범위는 `A00 현재 행동 유지`, `A04 도움 요청`, `A11 절도`로 제한한다. 다음 게이트는 필드 타입·정수식·RNG 키·행동 인스턴스 식별자·fixture와 시험을 고정하는 M4 세부 구현 명세다.
