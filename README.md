...
```text
STATUS = M0 PASS / M1 IMPLEMENTED
IMPLEMENTATION = FOUNDATION + STATE MODEL / SERIALIZATION
AUTOMATED_VERIFICATION = LOCAL PASS / LINUX + WINDOWS CI PENDING
...
M1 = IMPLEMENTED / FINAL CI PENDING
M2 = NOT AUTHORIZED
```
...
다음 단계인 M2 시간·자원 시스템은 별도의 구현 승인이 있기 전까지 시작하지 않는다.
